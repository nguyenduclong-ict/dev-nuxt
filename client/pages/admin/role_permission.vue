...
import { EntityPermission, Permission, Role } from '@/server/entities'
...
export default Vue.extend({
  ...
  async asyncData({ $axios, params, app }): Promise<Data> {
    const [entities, _apis, role, permissions] = await Promise.all([
      $axios.$get('/api/entity'),
      $axios.$get('/api/config/apis'),
      $axios.$get('/api/entity/roles/find-one', {
    ...
    const checker = new PermissionHelper()
    checker.permissions = role.permissions
    ...
    apis.forEach((api) => {
      api.permission = role.permissions.find(
        (item) =>
  ...
  methods: {
    ...
    async handleUpdate() {
      ...
      const role = await this.$axios.$put('/api/auth/update-role-permissions', {
        role: this.role.id,
        permissions,
      })
