...
export const useless = (v: any) => v

// Bearer Token
export const parseToken = (bearerToken: string) =>
  bearerToken.replace(/^bearer /i, '')
export const buildToken = (token: string) => `Bearer ${token}`
...
export function removeItems<T extends object>(
  arr: T[] = [],
  func: Partial<T> | ((item: T, index?: number) => boolean),
  keys: string | string[] | undefined = undefined
) {
 ...
 */
export function replace(arr = [], replaceBy: any, func: any) {
  if (typeof func !== 'function') {
  ...
  arr.splice(arr.findIndex(func), 1, replaceBy as never)
}
...
export function pushIfNotExists(
  arr: any[] = [],
  map: any,
  item: any,
  valueKey: string
 ...
 */
export function santizeResponse(response: any) {
 ...
 * @param  {...any} args
 */
export function getSourceValue(source: any, ...args: any[]) {
  if (typeof source === 'function') return source(...args)
  return source
}

export function getObjectId(value: any) {
...
export function joinUrl(...paths: string[]) {
  let url = paths
    .map((item) => {
      if (item === 'http://' || item === 'https://') return item
...
export function getAlias(tsConfig: any, baseUrl: any) {
  const alias: any = {}
...
export const cachedColor: any = {}

export function getRandomColor(source: any, a = 10) {
...
export function fileThumbnail(file: any, size: string) {
  const url = fileUrl(file)
  return url + '?thumnail=' + size
}
